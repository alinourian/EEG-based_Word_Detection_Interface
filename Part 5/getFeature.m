function new_Subject = getFeature(Subject,mode)
    if mode == 1
        new_Subject = Subject;
    end
    ...
    for i = 2:size(target_epoch,1)
         target_feature = cat(2,target_feature,squeeze(target_epoch(i,:,:)));
    ...
    for i = 2:size(not_target_epoch,1)
        not_target_feature = cat(2,not_target_feature,squeeze(not_target_epoch(i,:,:)));
    ...
    for i = 2:size(test_epoch,1)
         test_feature = cat(2,test_feature,squeeze(test_epoch(i,:,:)));
